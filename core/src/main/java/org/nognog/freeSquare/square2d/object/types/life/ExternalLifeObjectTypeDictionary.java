 ...
 */
public class ExternalLifeObjectTypeDictionary extends ExternalSquare2dObjectTypeDictionary<ExternalLifeObjectType> {
	...
	private boolean hasDuplicateNameType() {
		for (ExternalLifeObjectType type : this.getAllExternalObjectType()) {
	...
	protected void fixDictionaryToSavableState() {
		while (this.hasDuplicateNameType()) {
			Array<ExternalLifeObjectType> types = this.getAllExternalObjectType();
			for (int i = 0; i < types.size - 1; i++) {
				final ExternalLifeObjectType type = types.get(i);
				final int typeNameCount = this.countName(type.getName());
				if (typeNameCount < 2) {
					continue;
				}
				for (int j = i + 1; j < types.size; j++) {
					final ExternalLifeObjectType compareType = types.get(j);
