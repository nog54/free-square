...
public interface EatableObjectType extends Square2dObjectType<EatableObject> {
	...
	public static enum Prepared implements EatableObjectType {
		...
		private Prepared(Food bindFood, String texturePath, float logicalWidth, StatusInfluence... influences) {
			this(bindFood, texturePath, logicalWidth, EatableObject.class, influences);
		}

		private Prepared(Food bindFood, String texturePath, float logicalWidth, Color color, StatusInfluence... influences) {
			this(bindFood, texturePath, logicalWidth, color, EatableObject.class, influences);
		}

		private <T extends EatableObject> Prepared(Food bindFood, String texturePath, float logicalWidth, Class<T> klass, StatusInfluence... influences) {
			this(bindFood, texturePath, logicalWidth, Colors.WHITE, klass, influences);
		}

		private <T extends EatableObject> Prepared(Food bindFood, String texturePath, float logicalWidth, Color color, Class<T> klass, StatusInfluence... influences) {
			this(bindFood, new Texture(texturePath), logicalWidth, color, klass, influences);
		}

		@SuppressWarnings("unchecked")
		private <T extends EatableObject> Prepared(Food bindFood, EatableObjectType type, Color color, StatusInfluence... influences) {
			this(bindFood, type.getTexture(), type.getLogicalWidth(), color, (Class<T>) type.getSquareObjectClass(), influences);
		}

		@SuppressWarnings("unchecked")
		private <T extends EatableObject> Prepared(Food bindFood, EatableObjectType type, float logicalWidth, Color color, StatusInfluence... influences) {
			this(bindFood, type.getTexture(), logicalWidth, color, (Class<T>) type.getSquareObjectClass(), influences);
		}

		private <T extends EatableObject> Prepared(Food bindFood, Texture texture, float logicalWidth, Color color, Class<T> klass, StatusInfluence... influences) {
	...
	public static class Manager {
		public static void dispose() {
			for (EatableObjectType type : Prepared.values()) {
		...
		public static EatableObjectType[] getAll() {
			...
			return EatableObjectType.Prepared.values();
		}
	}
